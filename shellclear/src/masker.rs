    ...
    pub fn mask_sensitive_findings(&self, results: &mut [Command]) {
        ...
        for command in commands_with_detections {
            let should_mask = command.secrets.iter().any(|secret| {
                ...
                // there can still be an false-positive here, if the text is all made up of asterisks
                !command.command.contains(&replaced_secret)
            });

            if !should_mask {
                // If all the secrets are already masked, set the detections to an empty vec, this represents that this command is already masked / no secrets detected
